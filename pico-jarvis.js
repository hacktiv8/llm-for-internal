...
Observation: -2.8276 EUR for 1 USD.
Answer: The current exchange rate is -2.8276 EUR for 1 USD.
...
Now it your turn to answer the following!
...
async function exchange(from, to) {
    ...
    return `As per ${data.time_last_update_utc}, 1 ${from} is ${Math.ceil(rate)} ${to}`;
}

function parse(text) {
    const parts = {};
    const MARKERS = ['Answer', 'Observation', 'Thought', 'Action'];
...
async function reason(document, history, inquiry) {
    ...
    const HISTORY_MSG = "Before formulationg a thought, consider the following conversation history:";
    ...
    const prompt = SYSTEM_MESSAGE.replace("{{CONTEXT}}", context(history)).replace("{{QUESTION}}", inquiry);
...
You are given a {{KIND}}, and then you respond to the question.
...
Question: {{QUESTION}}
Thought: Let use the above reference to find the answer.
Answer:`
...
async function answer(kind, passages, question) {
    ...
    console.log("  fn answer llama(input):", output);
...
async function lookup(document, question, hint) {
    ...
    const top = refs.slice(0, 1).shift();
...
async function act(document, question, action, observation) {
    ...
    console.error('Not recognize action', name, arg);
...
async function ingest(url) {
    const input = await readPdfPages({ url: url });
    ...
    const isPunctuator = (ch) => (ch === ".") || (ch === "?") || (ch === "!") || (ch === ",");
    ...
    const pagination = sequence(pages.length)
                        .map(k => pages.slice(0, k + 1)
    ...
    const server = http.createServer(async (req, res) => {
        ...
        if ( url === '/health' ) {
            ...
            if (question === "!reset") {
                ...
                res.writeHead(200).end("History has been reset");
                return;
            }
            ...
            const {thought, action, observation, answer, source, reference} = await reason(document, state.history, `Question: ${question}`);
